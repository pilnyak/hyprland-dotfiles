...
execute_theme_scripts() {
    ...
    # Order matters:
    # 1. waybar-detection modifies the waybar TEMPLATE based on wallpaper luminosity
    # 2. wallust uses that template to generate colors.css files
    # 3. wofi colors update
    # 4. GTK update LAST - triggers gsettings reload after all colors are in place
...
reload_system_components() {
    ...
    reload_swaync
    restart_hyprswitch
    # reload_hyprland_plugins  # Disabled - no plugins installed
